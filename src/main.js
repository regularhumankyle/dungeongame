...
function movePlayer(dx, dy) {
    ...
    if (p.downed) { log("You are unconscious and cannot move."); return; }
    ...
    if (newCard && newCard.type === 'corridor') {
        p.lastCorridor = { x: tx, y: ty };
    }
...
function resolveToken(token, p, key) {
    ...
    if (token.type === 'loot') {
        ...
        if(event.effect.includes("Damage")) {
             p.hp = Math.max(0, p.hp - 1);
             if (p.hp <= 0) handlePlayerDeath(p, gameState.currentPlayerIndex);
        } else if (event.effect.includes("Turn Ends")) {
             p.moves = 0;
        }
...
function handlePlayerDeath(p, index) {
    if (gameState.players.length === 1) {
        document.getElementById("modal-title").innerText = "YOU DIED";
        document.getElementById("modal-title").className = "text-2xl font-bold mb-2 text-red-600";
        document.getElementById("modal-description").innerText = "Your journey has ended here.";
        document.getElementById("modal-content").innerHTML = "What would you like to do?";
        
        const opts = document.getElementById("modal-options");
        opts.innerHTML = "";
        
        const btnContinue = document.createElement("button");
        btnContinue.className = "w-full bg-blue-700 hover:bg-blue-600 p-3 rounded text-white font-bold";
        btnContinue.innerText = "Continue (Spawn at Last Corridor)";
        btnContinue.onclick = () => {
             p.hp = Math.floor(p.maxHp / 2);
             if (p.lastCorridor) {
                 p.x = p.lastCorridor.x;
                 p.y = p.lastCorridor.y;
             } else {
                 p.x = 0; p.y = 0; 
             }
             ...
             updateUI();
             centerCameraOnPlayer();
             log("You wake up wounded in a safe place...");
        };
        
        const btnQuit = document.createElement("button");
        btnQuit.className = "w-full bg-red-900 hover:bg-red-800 p-3 rounded text-white font-bold";
        btnQuit.innerText = "Give Up (Main Menu)";
        btnQuit.onclick = () => location.reload();

        opts.appendChild(btnContinue);
        opts.appendChild(btnQuit);
        ...
        p.downed = true;
        p.moves = 0;
        log(`${p.class.name} has fallen!`);
        updateUI();
    }
}

function handleRevive(targetIndex) {
    const target = gameState.players[targetIndex];
    const reviver = gameState.players[gameState.currentPlayerIndex];
    
    if (target.x !== reviver.x || target.y !== reviver.y) {
        target.x = reviver.x;
        target.y = reviver.y;
    }
    ...
    log(`${reviver.class.name} revived ${target.class.name}!`);
    updateUI();
}
...
function startGame() {
    ...
    gameState.players = ids.map((id, i) => ({
        ...
        gold: 5, moves: 1, x: 1, y: 1, inventory: [], 
        fledTokenKey: null, downed: false, lastCorridor: {x:0, y:0}
    }));
...
async function runCombatRound(action) {
    ...
    setTimeout(() => {
        ...
        if(p.hp <= 0) { 
            ...
            setTimeout(() => {
                endCombatRound();
                handlePlayerDeath(p, gameState.currentPlayerIndex);
            }, 2000); 
        }
...
function endTurn() {
    ...
    p.moves = 1; 
    log(`Turn: ${p.class.name}`);
    updateUI(); centerCameraOnPlayer();
    
    const key = `${p.x},${p.y}`;
    const token = gameState.tokens[key];
    if (token && token.type === 'monster' && token.monsterData && !p.downed) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('revive-player', (e) => {
        handleRevive(e.detail.targetIndex);
    });
    ...
    // --- CAMERA & PANNING LOGIC (PC & TOUCH) ---
    const mapContainer = document.getElementById('map-container');
    const getDistance = (touches) => Math.hypot(touches[0].pageX - touches[1].pageX, touches[0].pageY - touches[1].pageY);
    
    const handleStart = (e) => {
        if (e.target.closest('.grid-space') || e.target.closest('.player-token') || e.target.closest('.token-marker') || e.target.tagName === 'BUTTON') return;
        
        if (e.touches && e.touches.length === 2) {
            gameState.isPanning = false;
            gameState.initialPinchDist = getDistance(e.touches);
            gameState.initialScale = gameState.camera.scale;
            return;
        }
        ...
        gameState.panStart = { 
            x: cx - gameState.camera.x, 
            y: cy - gameState.camera.y 
        };
    ...
    const handleMove = (e) => {
        if (e.touches && e.touches.length === 2 && gameState.initialPinchDist) {
            ...
            const scaleChange = currentDist / gameState.initialPinchDist;
            gameState.camera.scale = Math.min(Math.max(0.5, gameState.initialScale * scaleChange), 3.0);
            updateCamera();
            return;
        }
    ...
    const handleEnd = (e) => {
        ...
        if (e.touches && e.touches.length < 2) {
            gameState.initialPinchDist = null;
        }
    };
